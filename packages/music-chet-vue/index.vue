<template>
  <div>
    <transition name="dis_list">
      <div class="mask">
        ...
        <div class="list_box" v-if="allMusicState.showAllBox">
          ...
          <div style="min-width: 100%;padding: 40px;overflow: auto;">
            <div>
              <div class="list_title">
                <span style="font-size: 14px;color: red;">“网抑云”Music 新歌榜的</span>
                <img :src=" allMusicState.musicStateButton" class="music_state"
                  @click="allMusicState.MusicStateChange" />
                <div class="music_search_box">
                  <input type="text" class="music_search" v-model="allMusicState.musicSearchVal" placeholder="请勿搜索歌曲" />
              ...
              <div class="music_ul_title">
                <span>歌曲</span><span>歌手</span>
              </div>
              <ul class="list">
                <li v-for="(item, index) in thisMusicList" :key="index" @mouseover="allMusicState.buttonActive(index)"
                  @dblclick="musicPlay((allMusicState.thisListPage - 1) * 5 + index)">
                  <div class="this_music_shlter" v-if="(allMusicState.thisListPage - 1) * 5 + index === thisMusicIndex">
                  </div>
    ...
    <!-- 音乐盒子 -->
    <div class="bbox" @click="addStyle()" :class="{ bbox_active: curPlayMusic.showBox }">
...
// 添加到我的歌曲
const ListAdd = (obj) => {
  getMusicInfo(obj.id).then((res: any) => {
    allMusicState.musicSearchVal = "";
    if (allMusicState.myMusicList.length === 0) {
      // 第一次搜索直接播放
      allMusicState.myMusicList = [res.data.songs[0]];
      getMusicType(-1);
    } else {
      allMusicState.myMusicList.push(res.data.songs[0]);
      //提示已经添加进去
    }
    MusicAlert("添加成功");
  });
...
const getMusicType = (id) => {
  if (thisMusicType.value !== id) {
    ...
    if (id === -1) {
      ...
      getHotMusic(id).then((res: any) => {

        allMusicState.musicList = res.data.playlist.tracks.splice(0, 200);
        thisMusicType.value = id;
        thisMusicIndex.value = 0;
        allMusicState.thisListPage = 1;
        getInfo();
...
const getInfo = () => {
  ...
  getMusicUrl(allMusicState.musicList[thisMusicIndex.value].id).then((res: any) => {
    ...
    if (
      ...
      curPlayMusic.musicUrl = res.data.data[0].url.replace("http://", "https://");
      ...
      getWords(allMusicState.musicList[thisMusicIndex.value].id).then((res: any) => {
      ...
      getHotTalk(allMusicState.musicList[thisMusicIndex.value].id).then((res: any) => {

        let count = 0;
        allMusicState.hotTalkList = res.data.hotComments.splice(0, 3);
...
const thisMusicList = computed(() => {
  return [...allMusicState.musicList].splice((allMusicState.thisListPage - 1) * 5, 5) //分页
}
)
// 监听搜索
watch(() => allMusicState.musicSearchVal, () => {
  if (allMusicState.musicSearchVal === "") {
    ...
    getSearchSuggest(allMusicState.musicSearchVal).then((res: any) => {
      if (res.data.result.songs === undefined) {
        allMusicState.musicSearchList = [];
      } else {
        allMusicState.musicSearchList = res.data.result.songs;
      }
    });
  }
})
